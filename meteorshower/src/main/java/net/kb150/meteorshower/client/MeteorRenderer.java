...
public class MeteorRenderer {
    ...
    // Lightmap-Konstanten für Tag/Nacht-Zyklus
    ...
    // ============================================================
    // DIREKTIONALER IMPACT (Passend zur Server-Physik)
    // ============================================================
    public static void addImpact(Vec3 pos, Vec3 incomingDir) {
        Random rand = new Random();
        
        // 1. SCHOCKWELLE (Flacher Ring am Boden)
        for (int i = 0; i < 30; i++) {
        ...
        // 2. DIREKTIONALE EJECTA (Material spritzt nach vorne)
        for (int i = 0; i < 50; i++) {
        ...
        // 3. THERMISCHE RAUCHSÄULE (Aufsteigend)
        for (int i = 0; i < 60; i++) {
    ...
    public static void onRenderLevel(RenderLevelStageEvent event) {
        ...
        // Wir nutzen die normale Projection-Matrix für Distant Horizons!
        ...
        float smoothTime = time + event.getPartialTick();
        ...
        for (ClientParticle smoke : ACTIVE_SMOKE_NODES) {
            ...
            // Atmospheric Fog: Rauch blendet auf Distanz aus
            float fogFactor = (float) Math.max(0.0, 1.0 - (dist / 8000.0));
            float currentSize = smoke.baseSize + (progress * 50.0F);
            float alpha = (1.0F - progress) * 0.6F * fogFactor;
            
            // Rauch wird mit der Zeit heller (Dampf)
            float color = Math.min(1.0F, 0.3F + progress * 0.7F);
            
            switchTexture(tesselator, buffer, smoke.texture);
            renderQuad(buffer, relativePos, right, up, currentSize, color, color, color, alpha, smoke.rotation + (progress * 1.5F), LIGHT_SKY_ONLY, farPlane);
        }

        // ========================================================
        // 2. FEUER & METEORE RENDERN (Immer Hell)
        // ========================================================
        RenderSystem.blendFunc(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE);
        switchTexture(tesselator, buffer, TEX_METEOR_1);

        // A. Impact-Feuer
        for (ClientParticle fire : ACTIVE_FIRE_NODES) {
            ...
            renderQuad(buffer, relativePos, right, up, currentSize, 1.0F, 0.7F, 0.3F, alpha, fire.rotation + (progress * 2.0F), LIGHT_FULL_BRIGHT, farPlane);
        }

        // B. Meteor Köpfe & Bow Shock (Ablation)
        for (MeteorClientHandler.ClientMeteor meteor : activeMeteors) {
            Vec3 cometPos = meteor.getInterpolatedPosition(event.getPartialTick());
            ...
            // --- BOW SHOCK SPAWNING (Vor dem Meteor) ---
            ...
            if (lastPos == null || lastPos.distanceToSqr(cometPos) > 1.0D) {
                ...
                for (int s = 0; s < 2; s++) {
                    ...
                    int lifespan = 800 + (int)(Math.random() * 400); // Sehr hohe Lebensdauer
            ...
            for (int i = 0; i < spriteCount; i++) {
                ...
                renderQuad(buffer, relativePos, right, up, size, 1.0F, 0.85F, 0.6F, 0.8F * fogFactor, rot, LIGHT_FULL_BRIGHT, farPlane);
            }
            
            // Haupt-Glow
            switchTexture(tesselator, buffer, TEX_METEOR_3);
            renderQuad(buffer, relativeCometPos, right, up, 35.0F, 1.0F, 0.5F, 0.1F, 0.6F * fogFactor, 0.0F, LIGHT_FULL_BRIGHT, farPlane);
        }
    ...
    // ============================================================
    // BILLBOARD QUAD MIT DISTANCE SCALING & LIGHTMAP
    // ============================================================
    private static void renderQuad(BufferBuilder buffer, Vec3 relativePos, Vector3f right, Vector3f up, float size, float r, float g, float b, float a, float rotationAngle, int lightmap, float farPlane) {
        double dist = relativePos.length();
        float maxDist = farPlane * 0.9F;

        // Distance Scaling für DH-Kompatibilität
        if (dist > maxDist) {
